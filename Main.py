...
def game(mode="normal"):
    ...
    money = 200  # Starting amount of money
    ...
    # Create buttons
    witchButton = Button(620, 95, IMAGES["witchSample"], True, "Witch",
                         tooltip_text="Witch: \"Double, double, toil and trouble!\" The Witch Tower hurls magical fireballs at            \nenemies, making them regret stepping onto your turf. \n- \"A reliable all-rounder tower that does decent damage\"\ncost: 100\natk: 10\n")  # (x, y, image, single_click, tower_name, tool_tip)
    archerButton = Button(680, 90, IMAGES["archerSample"], True, "Archer",
                          tooltip_text="Archer: \"Bullseye!\" The Archer Tower is your go-to for precision strikes. With its rapid         \n-fire arrows, it’s perfect for thinning out waves of weaker enemies. \n- \"A fast shooting arrows tower that does less damage\" \ncost: 80 \natk: 8\n")
    bearButton = Button(620, 140, IMAGES["bearSample"], True, "Bear",
                        tooltip_text="Bear: \"Rawr!\" The Bear Tower smashes enemies with brute force. It’s slow but devastating, \nperfect for taking down a group of foes. \n- \"A powerful tower that smashes the ground, dealing damage to all enemies in its range.\"\ncost: 120\natk: 25\n")
    slimeButton = Button(680, 145, IMAGES["slime"], True, "Slime",
                         tooltip_text="Slime: \"Sticky situation!\" The Slime Tower slows enemies with gooey projectiles, giving    \nyour other towers more time to finish the job. Great for controlling the battlefield \n- \"A tower that slows all enemies in its range.\" \ncost: 120\natk: slow\n")
    ...
    while running:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if upgrade_button_rect.collidepoint(event.pos):
                        ...
                        if selected_tower.upgrade == 3:
                            ...
                            log_message("Upgrades are disabled in No Upgrades Mode!")
                                ...
                                towers_base[selected_tower.tower_name]["upgrades"][selected_tower.upgrade + 1][
                                    "cost"] * (1.5 if mode == "hardcore_mode" else 1)):
                            cost = int(towers_base[selected_tower.tower_name]["upgrades"][selected_tower.upgrade + 1][
                                           "cost"] * (1.5 if mode == "hardcore_mode" else 1))
                            money -= cost
                            selected_tower.do_upgrade(game_state)
                            game_state["upgrades_used"] += 1
                            game_state["no_upgrades_wave"] = False
                            check_achievements(game_state, achievement_notifications, modes_played_global,
                                               modes_won_global)
                ...
                if placing_tower and temporary_tower:
                    ...
                    if mouse_x < 600 and mouse_y < 400:  # Ensure placement is within the map area
                        if (grid_x, grid_y) not in tower_positions and (
                        grid_x, grid_y) not in ENEMY_PATHS:  # Check if the position is free
        ...
        for enemy in enemies[:]:
            if not enemy.reached_end:
                ...
                money += floor(enemy.money / 2)  # increase money if enemy reaches end
                score -= enemy.score
                if lives <= 0:
        ...
        if not enemies and spawned_count == len(current_wave_enemies):
            if wave_number == FINAL_WAVE:  # game clear after 10 wave
                ...
                log_message(
                    f"Time increases your score by {((wave_number ** 3) * (200 / ((wave_end_time - wave_start_time) / 1000)))}")
                score += int(((wave_number ** 3) * (200 / ((
                                                                       wave_end_time - wave_start_time) / 1000))))  #The faster you beat the wave the more points you get
                total_wave_time += ((
                                                wave_end_time - wave_start_time) / 1000)  #Add time it took to beat the wave to the total time
                ...
                check_achievements(game_state, achievement_notifications, modes_played_global, modes_won_global)
                gameclear_screen(screen, score, SCORE_FILE, high_score, top_five, total_wave_time, time_high_score,
                                 time_top_five)
        ...
        for i, (name, ts) in enumerate(achievement_notifications):
            ...
            pygame.draw.rect(screen, border_color, (banner_x, banner_y, banner_width, banner_height), 2,
                             border_radius=8)
