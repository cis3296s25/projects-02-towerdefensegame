...
def returntohome(screen):
    ...
    popup_width = 300
    popup_height = 150
    popup_rect = pygame.Rect((screen.get_width() - popup_width) // 2, (screen.get_height() - popup_height) // 2,
...
def settings_screen(screen, in_game=False):
    ...
    sfx_handle_rect = pygame.Rect(sfx_slider_rect.x + int(sfx_slider_rect.width * Settings.sfx_volume) - 5,
                                  sfx_slider_rect.y - 5, 10, 20)
    ...
    while running:
        ...
        if sfx_filled_width > 0:
            pygame.draw.rect(screen, (255, 255, 255),
                             (sfx_slider_rect.x, sfx_slider_rect.y, sfx_filled_width, sfx_slider_rect.height))
        ...
        pygame.draw.rect(screen, (255, 255, 255), (
        sfx_slider_rect.x, sfx_slider_rect.y, int(sfx_slider_rect.width * Settings.sfx_volume), sfx_slider_rect.height))
...
def gameclear_screen(screen, score, SCORE_FILE, high_score, top_five, total_wave_time, time_high_score, time_top_five):
    ...
    if (high_score):
        score_text = pygame.font.Font("fonts/BrickSans.ttf", 40).render(f"HIGH SCORE: **{score}**", True,
                                                                        (138, 43, 226))
    elif (top_five):
    ...
    if (time_high_score):
        time_text = pygame.font.Font("fonts/BrickSans.ttf", 30).render(f"Time Taken: {total_wave_time}", True,
                                                                       (138, 43, 226))
    elif (time_top_five):
        time_text = pygame.font.Font("fonts/BrickSans.ttf", 30).render(f"Time Taken: {total_wave_time}", True,
                                                                       (173, 216, 23))
...
def gameover_screen(screen, score, SCORE_FILE, high_score, top_five):
    ...
    if (high_score):
        score_text = pygame.font.Font("fonts/BrickSans.ttf", 25).render(f"HIGH SCORE: **{score}**", True,
                                                                        (138, 43, 226))
    elif (top_five):
    ...
    quit_text = pygame.font.Font("fonts/BrickSans.ttf", 15).render("Press R to retry or CLICK to return to menu", True,
                                                                   (255, 255, 255))
...
def draw_sidebar(screen, lives, money):
    ...
    pygame.draw.line(screen, (255, 255, 255), (610, 80), (740, 80),
                     1)  # Draw a line below the Tower text (surface, color, start_pos, end_pos, width)
...
def draw_tower_stat(screen, tower, mode, money):
    ...
    if tower.upgrade < 3:
        text_final_total = font.render(
            f"Total - Cost: {money - towers_base[tower.tower_name]['upgrades'][tower.upgrade + 1]['cost']}", True,
            (255, 255, 255))
    else:
        text_final_total = font.render(
            f"Max Upgrade", True,
...
def mode_selection_screen(screen):
    ...
    start_y = 150
    button_rects = [
        pygame.Rect((screen.get_width() - button_width) // 2, start_y + i * (button_height + padding), button_width,
                    button_height)
